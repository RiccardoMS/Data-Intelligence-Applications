...
import numpy as np
# Import Margin
from Data_Exp import Mr
# Import Conversion Rate; Return probs; Click per day; Cost per click;
from Data_Exp import Cr_1,Cr_2,Cr_3,Cr_a
from Data_Exp import R_1,R_2,R_3,R_a
from Data_Exp import Cpc_1,Cpc_2,Cpc_3,Cpc_a
from Data_Exp import Ndc_1,Ndc_2,Ndc_3,Ndc_a
...
################################################ BRUTE FORCE SOLVER ####################################################
# Define Expected Revenue to be maximized
# Magari quando esce il 3.10
# def Expected_Revenue(price, bid, specific):
#   match specific:
#      case "Class1":
#         return Ndc_1(bid)*Cr_1(price)*margin(price)*(1+np.arry(C1['return probs'][0])*np.array(C1['return probs'][1])) \
#                - Ndc_1(bid)*Cpc_1(bid)
#      case "Class2":
#         return Ndc_2(bid) * Cr_2(price) * margin(price) * (
#                    1 + np.arry(C2['return probs'][0]) * np.array(C2['return probs'][1])) \
#                - Ndc_2(bid) * Cpc_2(bid)
#     case "Class3":
#         return Ndc_3(bid) * Cr_3(price) * margin(price) * (
#                    1 + np.arry(C3['return probs'][0]) * np.array(C3['return probs'][1])) \
#                - Ndc_3(bid) * Cpc_3(bid)
#      case "Aggregate":
#         return Ndc_a(bid) * Cr_a(price) * margin(price) * (
#                    1 + np.arry(aggregate['return probs'][0]) * np.array(aggregate['return probs'][1])) \
#                - Ndc_a(bid) * Cpc_a(bid)
#      case _:
#         return Ndc_a(bid) * Cr_a(price) * margin(price) * (
#                 1 + np.arry(aggregate['return probs'][0]) * np.array(aggregate['return probs'][1])) \
...
def Expected_Revenue_Class1(price, bid):
      return 0.3*(ndc_1(bid)*Cr_1(price)*Mr(price)*(1 + np.dot(np.array(C1['return probs'][0]),np.array(C1['return probs'][1]))) \
               - ndc_1(bid)*cpc_1(bid))

def Expected_Revenue_Class2(price, bid):
   return 0.5*(ndc_2(bid) * Cr_2(price) * Mr(price) * (1 + (1/30)*np.dot(np.array(C2['return probs'][0]),np.array(C2['return probs'][1]))) \
          - ndc_2(bid) * cpc_2(bid))

def Expected_Revenue_Class3(price, bid):
      return 0.2*(ndc_3(bid) * Cr_3(price) * Mr(price) * (
                 1 + (1/30)*np.dot(np.array(C3['return probs'][0]),np.array(C3['return probs'][1]))) \
             - ndc_3(bid) * cpc_3(bid))

def Expected_Revenue_Aggregated(price, bid):
   return ndc_a(bid) * Cr_a(price) * Mr(price) * (1 + (1/30)*np.dot(np.array(aggregate['return probs'][0]),np.array(aggregate['return probs'][1]))) \
          - ndc_a(bid) * cpc_a(bid)
