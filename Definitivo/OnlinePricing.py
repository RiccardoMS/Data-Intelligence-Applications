...
if __name__=="__main__":
 ...
 plt.xlabel("t")
 plt.ylabel("Regret")
 plt.plot(np.cumsum(np.mean(opt_value - np.array(ts_revenue_per_experiment), axis=0)), 'r')
 plt.plot(np.cumsum(np.mean(opt_value - np.array(ucb1_revenue_per_experiment), axis=0)), 'b')
 plt.legend(["TS", "UCB1"])
 ...
 ax1.plot(np.repeat([opt_value],T), 'r', label='Optimal Aggregated')
 ax1.set_ylim([0,800])
 ax1.set_xlabel('t')
 ...
 ax2.plot(np.repeat([opt_value], T), 'r', label='Optimal Aggregated')
 ax2.set_ylim([0, 800])
 ax2.set_xlabel('t')
